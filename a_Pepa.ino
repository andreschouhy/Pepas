class Pepa
{
  public:
    Pepa(uint8_t _puertoCV, uint8_t _puertoT, uint8_t _puertoCV2, uint8_t _puertoG, int _pote, uint8_t _modoSqrEnv, uint8_t _id)
    {
      ...
      puertoG = _puertoG;
      futureMillis = futureMillisT = escalaSize = escalaSizePreset1 = escalaSizePreset2 = escalaSizePreset3 = escalaSizePreset4 = 0;
      ...
      //velocidad  = (long)_pote * precision; //esto esta comentado para reemplazar velocidad individual por general
      velocidad  = velocidadGeneral;
      numero = 0;
      multiplicador = multiplicadorTemporal = 1;
    }
    
    uint8_t escalaSize, escalaSizePreset1, escalaSizePreset2, escalaSizePreset3, escalaSizePreset4, tamanoEscalaLimite, mantener, control, dividiendo, octava, modoSqrEnv, id, disparar;
    long multiplicador, velocidad, timingCap;
    int probabilidad, mutacion;
    int8_t _multiplicadorTemporal;
    uint8_t numero;
    ...
    void actualizar()
    {
      ...
      if (diferencia > 0) 
      {
        for(unsigned long d = 1; d <= diferencia; d++)
        {
          if(dividiendo == 0) timingCap += (velocidad * multiplicador);
          else if(dividiendo == 1) timingCap += (velocidad / multiplicador);
      ...
      if(modoSqrEnv == 1 && timingCap > sqrEnvCycle) digitalWrite(puertoG, LOW);
    ...
    void agregar(uint8_t tecla)
    {
      if(K2Midi(tecla) > 0)
      {
        if(escalaSize > 0 && mantener == 1 && notasPresionadas == 1) resetearEscala(); //resetear escala cuando esta en mantener
        escalaSize++;
    ...
    void quitar(uint8_t tecla)
    {
      if(mantener == 0)
      {
        ...
        while (index > -1)
        {
          for (int8_t i = index; i < escalaSize; i++) escala[i] = escala[i+1];
    ...
    void resetearEscala()
    {
      for(uint8_t i = 0; i < escalaSize; i++) escala[i] = 0;
      escalaSize = 0;
    }
    
    void mantenerSwitch()
    {
      if (mantener == 0) mantener = 1;
      else 
      {
        mantener = 0;
        resetearEscala();
    ...
    void resetearSecuencia()
    {
      ...
      for(uint8_t i = 0; i < secuenciaCant; i++) 
      {
        ...
        if(random(1024) <= probabilidad) secuencia[i][1] = 1;
        else secuencia[i][1] = 0;
      }
    }
    ...
    void mutarSecuencia()
    {
      if(random(1024) < mutacion) 
      {
        ...
        if(random(1024) <= probabilidad) secuencia[mutado][1] = 1;
        else secuencia[mutado][1] = 0;
      }
    }
    ...
    void controlarCTRL(uint8_t estado)
    {
      ...
      {
        control = 0;
        if(numero != 0) secuenciaCantTemp = numero;
        if (secuenciaCantTemp > 64) secuenciaCantTemp = 64; // limite para la cantidad de pasos de la secuencia
        numero = 0;
      }
    }
    ...
    void controlarALT(uint8_t estado)
    {
      ...
      {
        control = 0;
        if(numero != 0) notasSec = numero;
        if (notasSec > 16) notasSec = 16; // limite para la cantidad de notas posibles, dictado por el tamaño de escala[]
        numero = 0;
      }
    }
    ...
    unsigned long futureMillis, futureMillisT, sqrEnvCycle;
    
    void triggerLoop()
    {
      if(futureMillisT <= currentMillis && futureMillis != 0) 
      {
        futureMillisT = 0;
        digitalWrite(puertoT, LOW);
      }
